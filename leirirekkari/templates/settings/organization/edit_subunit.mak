	...
	<div id="content" class="row">
		<div class="ten columns">
			<form method="POST" action="">
				<fieldset>
					<legend>${_(u"Sub unit")}</legend>
					<label>
						${_(u"Name")}<br />
						<input type="text" name="name" value="${helpers.decodeString(subunit['name'])}" />
					</label>
					<label>
						${_(u"Short name")}<br />
						<input type="text" name="short_name" value="${helpers.decodeString(subunit['short_name'])}" />
					</label>
					<label>
						${_(u"Village")}<br />
						<select name="village_id">
							% for village in villages:
								% if village.id == subunit['village_id']:
