	...
	<div id="content" class="row">
		<div class="ten columns">
			<form method="POST" action="">
				<fieldset>
					<legend>${_(u"Club")}</legend>
					<label>
						${_(u"Name")}<br />
						<input type="text" name="name" value="${helpers.decodeString(club['name'])}" />
					</label>
					<label>
						${_(u"Short name")}<br />
						<input type="text" name="short_name" value="${helpers.decodeString(club['short_name'])}" />
					</label>
					<label>
						${_(u"Club code")}<br />
